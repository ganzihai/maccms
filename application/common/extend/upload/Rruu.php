...
class Rruu
{
    ...
    {
        $api = $GLOBALS['config']['upload']['api']['rruu']['api'];
        if (class_exists('CURLFile')) {     // php 5.5
            $post['image'] = new \CURLFile(realpath($filePath));
        } else {
            $post['image'] = '@' . realpath($filePath);
        }
        $post['token'] = $GLOBALS['config']['upload']['api']['rruu']['token'];
        $post['apiType'] = $GLOBALS['config']['upload']['api']['rruu']['apiType'];
        // 创建一个新 cURL 资源
        $curl = curl_init();
        // 设置URL和相应的选项
        // 需要获取的 URL 地址
